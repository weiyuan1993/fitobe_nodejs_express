	...
	$.ajax({
		...
		success:function(data){
			...
			if(articles.length==0){
				$(".search-block>p").text("查無資料");
			}else{

				if(search_content!=''){
					$(".search-block>p").text("關於"+search_content+"的文章共有"+articles.length+"篇");
				}else{
					$(".search-block>p").text("關於"+category+"的文章共有"+articles.length+"篇");	
				}
...
function addOtherPage(page){
	for(var i =1; i <= page.lastPage; i++){
		$(".pagination").append("<li><a class='next' page="+i+">第"+i+"頁"+"</a></li>");
	}
	$(".next").click(function(){
